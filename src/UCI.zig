...
pub fn position(board: *brd.Board, tokens: []const u8) !void {
    var split = std.mem.splitSequence(u8, tokens, " ");
    ...
    while (next_string != null) {
        if (std.mem.eql(u8, next_string.?, "perft")) {
            ...
            if (depth_str) |d| {
                ...
                const pos = try perft.perft(board, depth, depth, board.sideToMove, allocator);
                ...
                const diff_f: f32 = @as(f32, @floatFromInt(diff)) / 1000.0;
                const node_f: f32 = @as(f32, @floatFromInt(pos.Nodes));
                try std.io.getStdOut().writer().print("\nNPS: {} \n\n", .{@as(usize, @intFromFloat(node_f / diff_f))});
            }
            return;
        }
